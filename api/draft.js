...
export default async function handler(req, res) {
    ...
    try {
        ...
        กฎหลักในการตอบ:
        1. **กรณีแนะนำสินค้า (สำคัญมาก)**: ถ้าลูกค้าถามหากล้องประเภทใดก็ตามแบบกว้างๆ หรือยังไม่ได้ระบุรุ่นที่แน่นอน ให้แจ้งว่าขอแจ้งรายละเอียด และราคาสินค้าก่อน โดยห้ามเดารุ่น/ราคาเอง ให้ใช้รูปแบบ:
        ...
        *หมายเหตุและข้อควรระวังสำคัญ*:
        ...
        // --- ส่วนที่ 3: เตรียม Prompt ส่งให้ OpenAI ---
        let userPrompt = "";
        if (adminDraft && adminDraft.trim().length > 0) {
            userPrompt = `[ประวัติแชทล่าสุด]:\n${history}\n\n[สาระสำคัญที่แอดมินร่างไว้]: "${adminDraft}"\n\nคำสั่ง: นำสาระสำคัญที่แอดมินร่างไว้มาขยายความและเกลาให้สละสลวยตามกฎ 6 ข้อ โดยต้องพิจารณาบริบทแชทเพื่อไม่ให้สื่อสารซ้ำซ้อน และตอบกลับเพียงข้อความเดียวเท่านั้น:`;
        } else {
            userPrompt = `[ประวัติแชทล่าสุด]:\n${history}\n\nคำสั่ง: วิเคราะห์สถานการณ์และร่างข้อความตอบกลับถัดไปเพียงข้อความเดียวตามกฎ 6 ข้อ (หากไม่จำเป็นให้ส่งค่าว่าง):`;
        }
